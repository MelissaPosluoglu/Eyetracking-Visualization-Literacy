...
# Folder containing all participant TSV files
...
for file in files:
    ...
    # Keep only fixations with a reasonable duration
    fix = fix[
        (fix[DURATION] >= 80) &
        (fix[DURATION] <= 1000)
    ]
...
# Keep only participants with IDs from 21 to 30
result_df = result_df[
    (result_df["Participant_ID"] >= 21) &
    (result_df["Participant_ID"] <= 30)
]
...
pdf_file = "All_Participants_Report2.pdf"
...
for p in participants:
    ...
    # Extract total duration from the answers file
