...
RESULTS_BASE = os.path.join(BASE_DIR, "results", "testC")
...
def get_aois_q7():
    return [
        ...
        {"name": "answers", "x1": 0.314, "y1": 0.64, "x2": 0.685, "y2": 0.94, "type": "relevant"},
...
def plot_aoi_overlay(output_dir):
    ...
    for aoi in aois:
        ...
        if aoi["name"] == "question":
            tx, ty, ha, va = x1 + 6, y1 + 12, "left", "top"
        elif aoi["name"] == "y_axis":
            tx, ty, ha, va = x1 - 6, y1 + height / 2, "right", "center"
        elif aoi["name"] == "seoul":
            tx, ty, ha, va = x1 + width / 2, y1 + 27, "center", "bottom"
        elif aoi["name"] == "city_left":
            tx, ty, ha, va = x1 - 6, y1 + 12, "right", "top"
        elif aoi["name"] == "city_right":
            tx, ty, ha, va = x1 + width - 6, y1 + 12, "right", "top"
        elif aoi["name"] == "legend":
            tx, ty, ha, va = x1 + 12, y1 + 10, "left", "top"
        else:
            tx, ty, ha, va = x1 + 6, y1 + 12, "left", "top"

        plt.text(tx, ty, aoi["name"], fontsize=4, alpha=0.7, color=color, ha=ha, va=va)
